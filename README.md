...
- The default catalogue is provider-neutral and sorted newest publication first: XMA Original Release Date, Heliosphere package `createdAt`, and Nexus `created_timestamp` are normalized into one chronological timeline. Recently Updated uses each provider's update timestamp instead.
